...
/* Search all reltabs that we know about to locate ptr.  If found, set
   *psi to the relevant SegInfo, and *locno to the loctab entry number
   within that.  If not found, *psi is set to NULL.
*/
static void search_all_reltabs ( Addr ptr, /*OUT*/DebugInfo** pdi,
                                           /*OUT*/Int* locno )
{
   Int        lno;
   DebugInfo* di;
   ...
   for (di = debugInfo_list; di != NULL; di = di->next) {
      if (di->gotplt_present
          && di->gotplt_avma <= ptr
          && ptr < di->gotplt_avma + di->gotplt_size) {
         lno = ML_(search_one_reltab) ( di, ptr );
         if (lno == -1) goto not_found;
         *locno = lno;
         *pdi = di;
         return;
...
/* Map a code address to a function name of the jump slot.
   Returns True if successful. */
Bool VG_(get_jumpslot_fnname)( Addr a, Char* buf, Int nbuf )
{
   DebugInfo* di;
   Int        locno;
   search_all_reltabs ( a, &di, &locno );
   if (di == NULL)
      return False;
   VG_(strncpy_safely)(buf, di->reltab[locno].name, nbuf);
   return True;
}
