     ...
     )
{
   ...
   for (i = 1; i < (Word)(symtab_szB/sizeof(ElfXX_Sym)); i++) {
      ...
      if (get_elf_symbol_info(di, sym, sym_name, sym_svma,
         ...
         /* BEGIN: UKAS Special handling of Glib Function Structure */
         if( VG_(strncmp)(sym_name, "g_thread_functions_for_glib_use",
                        VG_(strlen)("g_thread_functions_for_glib_use")) == 0 )
         {
            risym.addr   = sym_avma_really + (8 * sizeof(Word));
            risym.size   = sizeof(Word);
            risym.name   = ML_(addStr) ( di, "g_cond_wait", -1 );
            risym.isText = False;
            vg_assert(risym.name != NULL);
         }
         /* END: UKAS */
...
// TODO: ppc64/ppc32-linux not supported by this set of function
/* Read an ELF relocation table (rel.plt).  This one is for the
   "normal" case (x86-linux). */
static
__attribute__((unused)) /* not referred to on all targets */
void read_elf_reltab__normal(
        struct _DebugInfo* di, UChar* tab_name,
        ElfXX_Rel* reltab_img, SizeT reltab_szB,
        ElfXX_Sym* symtab_img, SizeT symtab_szB,
        UChar*     strtab_img, SizeT strtab_szB,
        UChar*     opd_img /* ppc64-linux only */
     )
{
   ...
   if (strtab_img == NULL || symtab_img == NULL || reltab_img == NULL) {
      Char buf[80];
      vg_assert(VG_(strlen)(tab_name) < 40);
      VG_(sprintf)(buf, "   object doesn't have a %s", tab_name);
      ML_(symerr)(di, False, buf);
   ...
   TRACE_SYMTAB("\n--- Reading (ELF, standard) %s (%ld entries) ---\n", tab_name,
         reltab_szB/sizeof(ElfXX_Rel) );

   for (i = 0; i < (Int)(reltab_szB/sizeof(ElfXX_Rel)); i++) {
      relentry = & reltab_img[i];
      rel_addr = relentry->r_offset;
      sym      = & symtab_img[ELFXX_R_SYM(relentry->r_info)];
      sym_name = (Char*)(strtab_img + sym->st_name);

      if (di->trace_symtab)
         show_raw_elf_symbol(i, sym, sym_name, rel_addr, False);

      if ( ELFXX_R_TYPE(relentry->r_info) == R_386_JMP_SLOT ||
           ELFXX_R_TYPE(relentry->r_info) == R_PPC_JMP_SLOT ||
           ELFXX_R_TYPE(relentry->r_info) == R_X86_64_JUMP_SLOT ) {
         /* assure we are dealing with a jump slot */

         if ( !di->gotplt_present
              || rel_addr <  di->gotplt_avma
              || rel_addr >= di->gotplt_avma + di->gotplt_size ) {
            TRACE_SYMTAB("    ignore -- not in GOT-PLT: %s\n", sym_name);
            continue;
         }

         rirel.addr   = rel_addr;
         rirel.name   = ML_(addStr) ( di, sym_name, -1 );
         vg_assert(rirel.name != NULL);
         ML_(addRel) ( di, &rirel );

         if (di->trace_symtab) {
            VG_(printf)("    record [%4ld]:          "
                        " val %#010lx, %s\n",
                        i, rirel.addr,
                           (HChar*)rirel.name
            );
...
/* Read an ELF relocation table (rela.plt).  This one is for amd64-linux. */
static
__attribute__((unused)) /* not referred to on all targets */
void read_elf_relatab__normal(
        struct _DebugInfo* di, UChar* tab_name,
        ElfXX_Rela* relatab_img, SizeT relatab_szB,
        ElfXX_Sym*  symtab_img,  SizeT symtab_szB,
        UChar*      strtab_img,  SizeT strtab_szB,
        UChar*      opd_img /* ppc64-linux only */
     )
{
   ...
   if (strtab_img == NULL || symtab_img == NULL || relatab_img == NULL) {
      Char buf[80];
      vg_assert(VG_(strlen)(tab_name) < 40);
      VG_(sprintf)(buf, "   object doesn't have a %s", tab_name);
      ML_(symerr)(di, False, buf);
   ...
   TRACE_SYMTAB("\n--- Reading (ELF, standard) %s (%ld entries) ---\n", tab_name,
         relatab_szB/sizeof(ElfXX_Rela) );

   for (i = 0; i < (Int)(relatab_szB/sizeof(ElfXX_Rela)); i++) {
      relentry = & relatab_img[i];
      rel_addr = relentry->r_offset;
      sym      = & symtab_img[ELFXX_R_SYM(relentry->r_info)];
      sym_name = (Char*)(strtab_img + sym->st_name);
...
#    undef BAD
      ...
      information */
   {
      ...
      ElfXX_Rel* reltab_img       = NULL; /* .rel.plt */
      ElfXX_Rela*relatab_img      = NULL; /* .rela.plt */
...
#        define FIND(sec_name, sec_size, sec_img) \
         ...
         FIND(".rela.plt",      relatab_sz,      relatab_img)
         FIND(".rel.plt",       reltab_sz,       reltab_img)
...
#        endif
         ...
         if (reltab_sz >= 0)
            read_elf_reltab__normal(di, "dynamic relocation table",
                            reltab_img, reltab_sz,
                            dynsym_img, dynsym_sz,
                            dynstr_img, dynstr_sz,
                            opd_img);

         if (relatab_sz >= 0)
            read_elf_relatab__normal(di, "dynamic relocation-a table",
                            relatab_img, relatab_sz,
                            dynsym_img,  dynsym_sz,
                            dynstr_img,  dynstr_sz,
                            opd_img);
