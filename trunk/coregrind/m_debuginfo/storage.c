...
/* Add a symbol to the symbol table.
*/
void ML_(addRel) ( struct _DebugInfo* di, DiRel* rel )
{
   UInt   new_sz, i;
   DiRel* new_tab;

   /* Ignore entries with no names. */
   if (rel->name == NULL) return;

   if (di->reltab_used == di->reltab_size) {
      new_sz = 2 * di->reltab_size;
      if (new_sz == 0) new_sz = 500;
      new_tab = ML_(dinfo_zalloc)( "di.storage.addRel.1",
                                   new_sz * sizeof(DiRel) );
      if (di->reltab != NULL) {
         for (i = 0; i < di->reltab_used; i++)
            new_tab[i] = di->reltab[i];
         ML_(dinfo_free)(di->reltab);
      }
      di->reltab = new_tab;
      di->reltab_size = new_sz;
   }

   di->reltab[di->reltab_used] = *rel;
   di->reltab_used++;
   vg_assert(di->reltab_used <= di->reltab_size);
}
...
Int ML_(search_one_reltab) ( struct _DebugInfo* di, Addr ptr )
{
   Addr a_mid;
   Int  mid,
        lo = 0,
   ...
   while (True) {
      ...
      if (lo > hi) return -1; /* not found */
      mid      = (lo + hi) / 2;
      a_mid = di->reltab[mid].addr;

      if (ptr < a_mid) { hi = mid-1; continue; }
      if (ptr > a_mid) { lo = mid+1; continue; }
      vg_assert(ptr == a_mid);
      return mid;
   }
}
