...
/* Translate the basic block beginning at NRADDR, and add it to the
   ...
   Translation output must be indifferent to the value of translating_ahead!
*/

static
UShort translate    ( ThreadId tid,
                      Addr64   nraddr,
                      Bool     debugging_translation,
                      Int      debugging_verbosity,
                      ULong    bbs_done,
                      Bool     allow_redirection,
                      T_Mode   translation_mode,
                      IRSB*    (*lookahead_instrumenter)(VgCallbackClosure*,
...
#  endif
   ...
   vta.host_bytes       = ( translation_mode == TM_Normal ) ? tmpbuf : tmpbuf2;
   ...
   { /* At this point we have to reconcile Vex's view of the
     ...
     IRSB*(*f)(VgCallbackClosure*,
               IRSB*,VexGuestLayout*,VexGuestExtents*,
               IRType,IRType)
       = ( translation_mode == TM_LookAhead ) ? lookahead_instrumenter : VG_(tdict).tool_instrument;
     IRSB*(*g)(void*,
               IRSB*,VexGuestLayout*,VexGuestExtents*,
               IRType,IRType)
   ...
   vta.instrument2      = need_to_handle_SP_assignment() && (translation_mode == TM_Normal)
                             ? vg_SP_update_pass
                             : NULL;
   vta.finaltidy        = VG_(needs).final_IR_tidy_pass && (translation_mode == TM_Normal)
...
#  endif
   ...
   if( translation_mode == TM_Normal )
      tres = LibVEX_Translate2 ( &vta, True );
   else
      tres = LibVEX_Translate2 ( &vta, False );
   ...
   vg_assert( vge.len[0] != 0 );
   return vge.len[0];
}

/* Stub */

static Bool  translating_ahead = False;
static ThreadId ta_tid;
static Bool  ta_debugging_translation;
static Int   ta_debugging_verbosity;
static ULong ta_bbs_done;
static Bool  ta_allow_redirection;
...
Bool VG_(translate) ( ThreadId tid,
                      ...
                      Bool     allow_redirection )
{
   vg_assert( translating_ahead == False );

   ta_tid                   = tid;
   ta_debugging_translation = debugging_translation;
   ta_debugging_verbosity   = debugging_verbosity;
   ta_bbs_done              = bbs_done;
   ta_allow_redirection     = allow_redirection;

   return ( 0 != translate ( tid,
                             nraddr,
...
UShort VG_(translate_ahead) ( Addr64   nraddr,
                              IRSB*    (*lookahead_instrumenter)(VgCallbackClosure*,
                                                                 IRSB*,VexGuestLayout*,VexGuestExtents*,
                                                                 IRType,IRType) )
{
   ...
   vg_assert( translating_ahead == False );
   translating_ahead = True;

   ret =  translate ( ta_tid,
                      nraddr,
                      ta_debugging_translation,
                      ta_debugging_verbosity,
                      ta_bbs_done,
                      ta_allow_redirection,
                      TM_LookAhead,
                      lookahead_instrumenter );

   translating_ahead = False;
