...
{	// HChar* xpre  = VG_(clo_xml) ? "  <what>" : "";
   ...
   if (!thr->announced) {
      if (thr->errmsg_index == 1) {
         ...
         if (VG_(clo_xml))
               VG_(message)(Vg_UserMsg, "  <thread>Thread #%d is the program's root thread</thread>", thr->errmsg_index);
            // Stick xpre and xpost on the front and back of the format string.
         else  
        VG_(message)(Vg_UserMsg, "Thread #%d is the program's root thread", thr->errmsg_index);
       
      } else {
         tl_assert(thr->created_at != NULL);
         if (VG_(clo_xml))
                        VG_(message)(Vg_UserMsg, "  <thread>Thread #%d was created</thread>",  thr->errmsg_index);
         else
        VG_(message)(Vg_UserMsg, "Thread #%d was created",   thr->errmsg_index);
...
static void hg_pp_msg( Char* xml_name, Error* err, const HChar* format, ... )
{
   HChar* xpre  = VG_(clo_xml) ? "  <what>" : "";
   HChar* xpost = VG_(clo_xml) ? "</what>"  : "";
   Char buf[256];
...
static void hg_pp_origin ( ExeContext* ec, const HChar* format, ... )
{
   /* HChar* src   = NULL; */
   HChar* xpre  = VG_(clo_xml) ? "  <what>" : " ";
   HChar* xpost = VG_(clo_xml) ? "</what>"  : "";
...
void HG_(pp_Error) ( Error* err )
{
   ...
   case XE_LockOrder: {
      ...
      announce_one_thread( xe->XE.LockOrder.thr );
          hg_pp_msg("Lock order violated", err,
   ...
   case XE_UnlockBogus: {
      ...
      hg_pp_msg("Thread unlocked an invalid lock", err,
   ...
   case XE_UnlockForeign: {
      ...
      hg_pp_msg("Thread #%d unlocked lock ", err,
                "at location %p "
    ...
    //  VG_(pp_ExeContext)( VG_(get_error_where)(err) );
         ...
         hg_pp_origin(xe->XE.UnlockForeign.lock->appeared_at,
   ...
   case XE_UnlockUnlocked: {
      ...
      announce_one_thread( xe->XE.UnlockUnlocked.thr );
        hg_pp_msg("Thread unlocked a not-locked lock ", err,
        ...
        //VG_(pp_ExeContext)( VG_(get_error_where)(err) );
         if (xe->XE.UnlockUnlocked.lock->appeared_at) {
          hg_pp_origin(xe->XE.UnlockUnlocked.lock->appeared_at,
                        "Lock at %p was first observed",
                        (void*)xe->XE.UnlockUnlocked.lock->guestaddr);
   ...
   case XE_FreeMemLock: {
      ...
      hg_pp_msg("Thread deallocated location ", err,
     ...
     // VG_(pp_ExeContext)( VG_(get_error_where)(err) );
      if (xe->XE.FreeMemLock.lock->appeared_at) {
         hg_pp_origin(xe->XE.FreeMemLock.lock->appeared_at,
                        "Lock at %p was first observed",
                        (void*)xe->XE.FreeMemLock.lock->guestaddr);
   ...
   case XE_Race: {
      ...
      hg_pp_msg("PossibleDataRace", err,
                "Possible data race during %s of size %d"
                " at %#lx by thread #%d",
                what, szB, err_ga, (Int)xe->XE.Race.thr->errmsg_index);
      VG_(pp_ExeContext)( VG_(get_error_where)(err) );
      if (xe->XE.Race.mb_confacc) {
         if (xe->XE.Race.mb_confaccthr) {
           hg_pp_origin(xe->XE.Race.mb_confacc,
                         "This conflicts with a previous %s"
                         " of size %d by thread #%d",
