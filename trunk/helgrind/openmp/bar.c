...
#include "libgomp.h"
#include "../hg_annotations.h"

void
gomp_barrier_init (gomp_barrier_t *bar, unsigned count)
{
  DO_CREQ_v_WW (_VG_USERREQ__HG_OMP_BARRIER_INIT_PRE, gomp_barrier_t*, bar, unsigned long, (unsigned long) count);
...
{
  DO_CREQ_v_W (_VG_USERREQ__HG_OMP_BARRIER_DESTROY_PRE, gomp_barrier_t*, bar);
...
{
  DO_CREQ_v_WW (_VG_USERREQ__HG_OMP_BARRIER_REINIT_PRE, gomp_barrier_t*, bar, unsigned long, (unsigned long)count);
  gomp_mutex_lock (&bar->mutex1);
  bar->total = count;
  gomp_mutex_unlock (&bar->mutex1);
}
...
{ 
  DO_CREQ_v_W (_VG_USERREQ__HG_OMP_BARRIER_WAIT_PRE, gomp_barrier_t*, barrier);
