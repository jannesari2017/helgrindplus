...
static UWord stats__msm_read_ExclRW_to_ShR   = 0;
static UWord stats__msm_read_ExclRW_to_ShM2  = 0;
static UWord stats__msm_read_ShR_to_ShR      = 0;
static UWord stats__msm_read_ShM_to_ShM      = 0;
static UWord stats__msm_read_ShM_to_ExclRW   = 0;
static UWord stats__msm_read_ShM2_to_Race    = 0;
static UWord stats__msm_read_ShM2_to_ExclRW  = 0;
static UWord stats__msm_read_ShM2_to_ShM2    = 0;
...
static UWord stats__msm_write_ExclRW_nochange = 0;
static UWord stats__msm_write_ExclRW_transfer = 0;
static UWord stats__msm_write_ExclRW_to_Race  = 0;
static UWord stats__msm_write_ExclRW_to_ShM2  = 0;
static UWord stats__msm_write_ShR_to_ShM      = 0;
static UWord stats__msm_write_ShR_to_ExclRW   = 0;
static UWord stats__msm_write_ShM_to_ShM      = 0;
static UWord stats__msm_write_ShM_to_ExclRW   = 0;
static UWord stats__msm_write_ShM2_to_Race    = 0;
static UWord stats__msm_write_ShM2_to_ExclRW  = 0;
static UWord stats__msm_write_ShM2_to_ShM2    = 0;
                              ...
                              Addr acc_addr, SizeT szB )
{
   ...
   /* Exclusive Read */
   if (LIKELY(SVal__is_ExclR(svOld))) {
      /* read ExclR(segid)
           |  segid_old == segid-of-thread
           -> no change
           |  segid_old `happens_before` segid-of-this-thread
   ...
   /* Exclusive ReadWrite */
   if (LIKELY(SVal__is_ExclRW(svOld))) {
      /* read ExclR(segid)
           |  segid_old == segid-of-thread
           -> no change
           |  segid_old `happens_before` segid-of-this-thread
      ...
      */
      ...
      if (ord == POrd_EQ || ord == POrd_LT) {
         ...
         WordSetID lset_acc = main_get_LockSet( acc_thr, /*allLocks*/ True );
         svNew = SVal__mk_ShR( vtsid_old, lset_acc );
         stats__msm_read_ExclRW_to_ShR++;
         goto out;
      }
      else {
         /* Enter the shared-Modified-2 (ShM2) state. */
         WordSetID lset_acc = main_get_LockSet( acc_thr, /*allLocks*/ True );
         svNew = SVal__mk_ShM2( vtsid_old, lset_acc );
         stats__msm_read_ExclRW_to_ShM2++;
         goto out;
      }
      /*NOTREACHED*/
   }
...
//         record_last_lock_lossage(a,lset_old,lset_new);
   ...
   /* Shared-Modified-1 */
   if (SVal__is_ShM(svOld)) {
     /* Refine the lockset accordingly.  Do not
        complain if the lockset becomes empty -- that's ok. */
      ...
      if ( HG_(isEmptyWS)(univ_lsets, lset_new) ) {
            svNew = SVal__mk_ExclRW(tvid);
            stats__msm_read_ShM_to_ExclRW++;
            goto out;
...
//         record_last_lock_lossage(a, lset_old, lset_new);
      ...
      if ( HG_(isEmptyWS)(univ_lsets, lset_new) ) {
         ...
         if (ord == POrd_EQ || ord == POrd_LT) {
            /* happens-before */
            svNew = SVal__mk_ExclRW(tvid);
            stats__msm_read_ShM2_to_ExclRW++;
            goto out;
         }
         else {
            svNew = SVal__mk_Race();
            record_race_info( acc_thr, acc_addr, szB, False/*!isWrite*/ );
            stats__msm_read_ShM2_to_Race++;
            goto out;
         }
         
      }
      else {
         svNew = SVal__mk_ShM2( tvid, lset_new );
         stats__msm_read_ShM2_to_ShM2++;
         goto out;
      }
   }
  ...
  out:
   ...
   if (svNew != svOld && HG_(clo_trace_addr) == acc_addr ) {
                              ...
                              Addr acc_addr, SizeT szB )
{
   ...
   if (SVal__is_ExclRW(svOld)) {
      ...
      */
      ...
      if (ord == POrd_EQ ) {
         /* no change */
         stats__msm_write_ExclRW_nochange++;
         svNew = svOld;
         goto out;
      }
      else if (ord == POrd_LT) {
         /* -> Excl(segid-of-this-thread) */
         svNew = SVal__mk_ExclRW(tvid);
         stats__msm_write_ExclRW_transfer++;
         goto out;
      }
      else {
         /* Enter the shared-modified-2 (ShM2) state. */
         ...
         if (HG_(isEmptyWS)(univ_lsets, lset_acc)) {
            svNew = SVal__mk_Race();
            record_race_info( acc_thr, acc_addr, szB, True/*isWrite*/ );
            stats__msm_write_ExclRW_to_Race++;
            goto out;
         }
         else {
            svNew = SVal__mk_ShM2( vtsid_old, lset_acc );
            stats__msm_write_ExclRW_to_ShM2++;
            goto out;
         }
         /*NOTREACHED*/
      }
   ...
   if (SVal__is_ShR(svOld)) {
      ...
      if (HG_(isEmptyWS)(univ_lsets, lset_new)) {
         svNew = SVal__mk_ExclRW( tvid );
         stats__msm_write_ShR_to_ExclRW++;
         goto out;
      }
      else {
...
//         record_last_lock_lossage(a, lset_old, lset_new);
      ...
      if ( HG_(isEmptyWS)(univ_lsets, lset_new) ) {
         svNew = SVal__mk_ExclRW(tvid);
         stats__msm_write_ShM_to_ExclRW++;
         goto out;
      }
      else {
         svNew = SVal__mk_ShM( tvid, lset_new );
...
//         record_last_lock_lossage(a, lset_old, lset_new);
      ...
      if ( HG_(isEmptyWS)(univ_lsets, lset_new) ) {
         ...
         if (ord == POrd_EQ || ord == POrd_LT ) {
            /* -> Excl(segid-of-this-thread) */
            svNew = SVal__mk_ExclRW(tvid);
            stats__msm_write_ShM2_to_ExclRW++;
            goto out;
         }
         else {
            svNew = SVal__mk_Race();
            record_race_info( acc_thr, acc_addr, szB, True/*!isWrite*/ );
            stats__msm_write_ShM2_to_Race++;
            goto out;
         }
         /*NOTREACHED*/
      }
